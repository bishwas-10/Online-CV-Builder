...
const Navbar: React.FC = () => {
  ...
  return (
    <div className=" px-20 min-w-full h-20 bg-gray-300 shadow-md flex flex-row items-center justify-between">
      <div>Logo</div>
      <div className="w-48 h-full flex flex-row items-center justify-between text-md">
        <span
        ...
        {showDiv && (
          <div
            onMouseEnter={() => setShowDiv(true)}
            onMouseLeave={() => setShowDiv(false)}
            className="w-[40%] h-max absolute top-20 left-[50%] -translate-x-[50%] bg-white shadow-md p-4"
          >
            <div className="w-full flex items-center justify-between px-2 font-medium">
                <span className="text-lg">Resume Template</span>
                <button className="text-blue-500 hover:text-blue-700 py-1 "> View all</button>

            </div>
            <div className="flex flex-wrap gap-4 w-full px-2 mt-4">
            {resumeTemplate.map((resume,index)=>{
                return <div key={index} className="w-[48%] h-max group cursor-pointer">
                    <div className="flex flex-row w-full justify-between items-center group-hover:text-blue-500">
                        <div className="flex gap-2 "><span><resume.icon/></span><span className="capitalize">{resume.name}</span></div>
                        <ChevronRight/>
                    </div>
