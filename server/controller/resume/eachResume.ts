  ...
  switch (method) {
    case "GET":
      try {
        ...
        if (!resume) {
          return res
            .status(404)
            .json({ success: false, error: "No such resume exist!" });
        }
    ...
    case "DELETE":
      try {
        ...
        const personal = await About.find({ resumeId: resume._id, userId });
    ...
    default:
      res
        .status(404)
        .json({ success: false, error: "This route does'nt exist" });
      break;
  }
};
